...
const FB = 'Paperlogy', FS = 'Paperlogy', FR = 'Paperlogy';
...
async function main() {
  ...
  // 1. cover
  await html2pptx(path.join(DIR, 'v2_slide1_cover_pdf.html'), pptx, { tmpDir: TMP });

  // 2. fit table
  const { slide: s2, placeholders: p2 } = await html2pptx(path.join(DIR, 'v2_slide2_table_pdf.html'), pptx, { tmpDir: TMP });
  ...
  // 3. charts
  const { slide: s3, placeholders: p3 } = await html2pptx(path.join(DIR, 'v2_slide3_chart_pdf.html'), pptx, { tmpDir: TMP });
  ...
  // 4. roadmap gantt
  const { slide: s4, placeholders: p4 } = await html2pptx(path.join(DIR, 'v2_slide4_roadmap_pdf.html'), pptx, { tmpDir: TMP });
  ...
  const { slide: s5, placeholders: p5 } = await html2pptx(path.join(DIR, 'v2_slide5_action_pdf.html'), pptx, { tmpDir: TMP });
  ...
  await pptx.writeFile({ fileName: path.join(DIR, '디자인샘플_v2pdf.pptx') });
  console.log('done');
}
