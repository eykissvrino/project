...
set "WORKSPACE=%~dp0"
set "TEMPLATE=%WORKSPACE%_core\templates\project-template"
...
REM 공유 스킬을 프로젝트에 복사
xcopy "%WORKSPACE%_core\skills" "!PROJECT_DIR!\.claude\skills" /E /I /Q >nul 2>nul
xcopy "%WORKSPACE%_core\skills" "!PROJECT_DIR!\.agent\skills" /E /I /Q >nul 2>nul

REM 공유 에이전트 교차 배포 (Claude Code + Antigravity)
if not exist "!PROJECT_DIR!\.claude\agents" mkdir "!PROJECT_DIR!\.claude\agents"
copy "%WORKSPACE%_core\agents\*.md" "!PROJECT_DIR!\.claude\agents\" /Y >nul 2>nul
copy "%WORKSPACE%_core\agents\*.md" "!PROJECT_DIR!\.agent\workflows\" /Y >nul 2>nul

REM 공유 커맨드 복사
copy "%WORKSPACE%_core\commands\*.md" "!PROJECT_DIR!\.claude\commands\" /Y >nul 2>nul

REM 공유 워크플로우 교차 배포 (Antigravity + Claude Code)
if not exist "!PROJECT_DIR!\.agent\workflows" mkdir "!PROJECT_DIR!\.agent\workflows"
copy "%WORKSPACE%_core\workflows\*.md" "!PROJECT_DIR!\.agent\workflows\" /Y >nul 2>nul
copy "%WORKSPACE%_core\workflows\*.md" "!PROJECT_DIR!\.claude\agents\" /Y >nul 2>nul
