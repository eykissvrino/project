...
```
워크스페이스 = 사무실
├── _core/      = 공용 자료실 (모든 부서가 가져다 쓰는 양식, 매뉴얼, 도구)
...
3. _core/prompts/claude-web/ 파일을 시스템 지침에 복사 (처음 1회)
4. 자유롭게 대화
```
...
```
1. 앱 실행
2. _core/prompts/cowork/general-assistant.md 내용을 시스템 프롬프트에 설정 (처음 1회)
3. 자유롭게 대화
```
...
| **동기화 (Sync)** | 공용 자료실(_core/)의 최신 내용을 모든 프로젝트에 반영하는 것 |
...
### Q: 새 스킬을 추가하고 싶어요
**A:** "OO 스킬 만들어줘"라고 말하세요. AI가 `_core/skills/` 에 새 스킬을 만들고 REGISTRY.md에 등록합니다.
...
**A:** Claude Web은 로컬 파일을 직접 못 읽어서, Projects 기능에 프롬프트를 복사해 넣어야 합니다. `_core/prompts/claude-web/` 파일을 시스템 지침에 한 번만 붙여넣으면 됩니다.
